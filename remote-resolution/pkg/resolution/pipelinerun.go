...
func (req *PipelineRunResolutionRequest) Resolve(ctx context.Context) error {
	...
	var pipelineMeta *metav1.ObjectMeta = nil
	var pipelineSpec *v1beta1.PipelineSpec = nil

	// gitReq, err := parseGitAnnotations(req.PipelineRun.Annotations)
	// if err == nil {
	// 	var err error
	// 	pipelineMeta, pipelineSpec, err = gitReq.Execute()
	// } else if errors.Is(err, &InvalidGitAnnotations{}) {
	// 	log.Printf("invalid git reference: %v", err.Error())
	// } else if errors.Is(err, &MissingGitAnnotation{}) {
	// 	log.Printf("%q: not a git reference: %v", pr.Name, err.Error())
	// 	log.Printf("%q: proceeding as if normal pipelineref", pr.Name, err.Error())
