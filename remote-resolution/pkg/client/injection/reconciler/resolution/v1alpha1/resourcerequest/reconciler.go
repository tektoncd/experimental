...
import (
	...
	versioned "github.com/tektoncd/experimental/remote-resolution/pkg/client/clientset/versioned"
	resolutionv1alpha1 "github.com/tektoncd/experimental/remote-resolution/pkg/client/listers/resolution/v1alpha1"
	zap "go.uber.org/zap"
...
// controller reconciling v1alpha1.ResourceRequest.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1alpha1.ResourceRequest. Any changes
	// to the objects .Status or .Finalizers will be propagated to the stored
	// object. It is recommended that implementors do not call any update calls
	// for the Kind inside of ReconcileKind, it is the responsibility of the calling
	// controller to propagate those properties. The resource passed to ReconcileKind
	// will always have an empty deletion timestamp.
	ReconcileKind(ctx context.Context, o *v1alpha1.ResourceRequest) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1alpha1.ResourceRequest.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1alpha1.ResourceRequest. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1alpha1.ResourceRequest) reconciler.Event
}

// ReadOnlyInterface defines the strongly typed interfaces to be implemented by a
// controller reconciling v1alpha1.ResourceRequest if they want to process resources for which
// they are not the leader.
type ReadOnlyInterface interface {
	// ObserveKind implements logic to observe v1alpha1.ResourceRequest.
	// This method should not write to the API.
	ObserveKind(ctx context.Context, o *v1alpha1.ResourceRequest) reconciler.Event
}

type doReconcile func(ctx context.Context, o *v1alpha1.ResourceRequest) reconciler.Event

// reconcilerImpl implements controller.Reconciler for v1alpha1.ResourceRequest resources.
type reconcilerImpl struct {
	...
	Lister resolutionv1alpha1.ResourceRequestLister
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister resolutionv1alpha1.ResourceRequestLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) Reconcile(ctx context.Context, key string) error {
	...
	getter := r.Lister.ResourceRequests(s.namespace)
...
func (r *reconcilerImpl) updateStatus(ctx context.Context, existing *v1alpha1.ResourceRequest, desired *v1alpha1.ResourceRequest) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {

			getter := r.Client.ResolutionV1alpha1().ResourceRequests(desired.Namespace)
		...
		updater := r.Client.ResolutionV1alpha1().ResourceRequests(existing.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1alpha1.ResourceRequest) (*v1alpha1.ResourceRequest, error) {

	getter := r.Lister.ResourceRequests(resource.Namespace)
	...
	patcher := r.Client.ResolutionV1alpha1().ResourceRequests(resource.Namespace)
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1alpha1.ResourceRequest) (*v1alpha1.ResourceRequest, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1alpha1.ResourceRequest, reconcileEvent reconciler.Event) (*v1alpha1.ResourceRequest, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
