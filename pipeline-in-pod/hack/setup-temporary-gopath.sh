...
function shim_gopath() {
  ...
  local TEMP_VDEMEESTER="${TEMP_GOPATH}/src/github.com/tektoncd"
  local TEMP_TASKGROUP="${TEMP_VDEMEESTER}/experimental/pipeline-in-pod"
  local NEEDS_MOVE=1
...
function delete_pipeline_repo_symlink() {
  ...
  local TEMP_GOPATH="${REPO_DIR}/.gopath"
  if [ -d "$TEMP_GOPATH" ] ; then
    local REPO_SYMLINK="${TEMP_GOPATH}/src/github.com/tektoncd/experimental/pipeline-in-pod"
