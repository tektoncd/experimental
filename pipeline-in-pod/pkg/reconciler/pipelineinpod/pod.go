...
func getPod(ctx context.Context, pr *v1beta1.PipelineRun, ps *v1beta1.PipelineSpec, images pipeline.Images, entrypointCache EntrypointCache) (*corev1.Pod, error) {
	activeDeadlineSeconds := int64(pr.GetTimeout(ctx).Seconds() * 1.5)
	...
	// TODO: sidecars
	scriptsInit, ptcs := convertScripts(images.ShellImage, "", ps)
	if scriptsInit != nil {
		initContainers = append(initContainers, *scriptsInit)
		volumes = append(volumes, scriptsVolume)
	}
	for _, ptc := range ptcs {
		stepContainers, err := resolveEntrypoints(ctx, entrypointCache, pr.Namespace, pr.Spec.ServiceAccountName, ptc.containers)
		if err != nil {
			return nil, err
		}

		for i, s := range stepContainers {
			stepContainers[i].Name = names.SimpleNameGenerator.RestrictLength(ContainerName(ptc.pt.Name, s.Name, i))
		}
	...
	annotations, err := getAnnotations(pr)
	if err != nil {
		return nil, err
	}
	// TODO: secrets/creds
	pod := &corev1.Pod{
		ObjectMeta: metav1.ObjectMeta{
			...
			Name:      kmeta.ChildName(pr.Name, "-pod"),
...
func convertScripts(shellImageLinux string, shellImageWin string, ps *v1beta1.PipelineSpec) (*corev1.Container, []pipelineTaskContainers) {
	placeScripts := false

	shellImage := shellImageLinux
	shellCommand := "sh"
	shellArg := "-c"
	requiresWindows := checkWindowsRequirement(ps)
	ptcs := make([]pipelineTaskContainers, len(ps.Tasks)) // task might have multiple containers: problem for later
...
func checkWindowsRequirement(ps *v1beta1.PipelineSpec) bool {
...
func getAnnotations(pr *v1beta1.PipelineRun) (map[string]string, error) {
	podAnnotations := kmeta.CopyMap(pr.Annotations)
	/*
...
func getLabels(pr *v1beta1.PipelineRun) map[string]string {
	labels := make(map[string]string, len(pr.ObjectMeta.Labels)+1)
	for k, v := range pr.ObjectMeta.Labels {
		labels[k] = v
	}
