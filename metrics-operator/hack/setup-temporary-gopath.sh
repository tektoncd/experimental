...
function shim_gopath() {
  local REPO_DIR="$(git rev-parse --show-toplevel)/metrics-operator"
  local TEMP_GOPATH="${REPO_DIR}/.gopath"
  local TEMP_EXPERIMENTAL="${TEMP_GOPATH}/src/github.com/tektoncd/experimental"
  local TEMP_METRICS_OPERATOR="${TEMP_EXPERIMENTAL}/metrics-operator"
  local NEEDS_MOVE=1
  ...
  ln -s "$REPO_DIR" "$TEMP_EXPERIMENTAL"
  echo "Moving to $TEMP_METRICS_OPERATOR"
  cd "$TEMP_METRICS_OPERATOR"
...
function delete_pipeline_repo_symlink() {
  ...
  local TEMP_GOPATH="${REPO_DIR}/.gopath"
  if [ -d "$TEMP_GOPATH" ] ; then
    local REPO_SYMLINK="${TEMP_GOPATH}/src/github.com/tektoncd/experimental/metrics-operator"
