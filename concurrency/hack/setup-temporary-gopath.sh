...
function shim_gopath() {
  local REPO_DIR="$(git rev-parse --show-toplevel)/concurrency"
  local TEMP_GOPATH="${REPO_DIR}/.gopath"
  local TEMP_EXPERIMENTAL="${TEMP_GOPATH}/src/github.com/tektoncd/experimental"
  local TEMP_CONCURRENCY="${TEMP_EXPERIMENTAL}/concurrency"
  local NEEDS_MOVE=1
  ...
  echo "Moving to $TEMP_CONCURRENCY"
  cd "$TEMP_CONCURRENCY"
...
function delete_pipeline_repo_symlink() {
  ...
  local TEMP_GOPATH="${REPO_DIR}/.gopath"
  if [ -d "$TEMP_GOPATH" ] ; then
    local REPO_SYMLINK="${TEMP_GOPATH}/src/github.com/tektoncd/experimental/concurrency"
