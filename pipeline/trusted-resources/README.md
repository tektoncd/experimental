...
This webhook only verify on `create` operation considering that annotations will
be mutated by other components and will fail the verification if signature is not
updated accordingly.

Fields not generated by users will also be excluded before verification. Including:
